...
static mixed_block_header_t *free_list_head = NULL;
static mixed_block_header_t *alloc_list_head = NULL;
int mixed_counter = 0;
 ...
 */
static int is_valid_allocated_pointer(mixed_block_header_t *target) {
    mixed_block_header_t *curr = alloc_list_head;
    while (curr != NULL) {
 ...
 */
static mixed_block_header_t* request_more_memory(size_t required_size) {
    ...
    mixed_block_header_t *new_block = (mixed_block_header_t *)mapped_region;
    new_block->size = mmap_size - MIXED_HEADER_SIZE;
    new_block->is_free = true;
    new_block->next_free = NULL;
    new_block->prev_free = NULL;

    // Add it in order of free list
    mixed_block_header_t *curr = free_list_head;
    mixed_block_header_t *prev = NULL;
...
int mixed_init(size_t initial_size) {
    ...
    free_list_head = (mixed_block_header_t *) heap_start;
    free_list_head->size = initial_size - MIXED_HEADER_SIZE;
...
void *mixed_first_fit_malloc(size_t size) {
    if (size <= 0) return NULL;

    size_t aligned_size = align4(size);
    size_t total_required = aligned_size + MIXED_HEADER_SIZE;
    mixed_block_header_t *curr = free_list_head;
    ...
    if (curr->size >= (aligned_size + MIXED_HEADER_SIZE + 4)) {
        mixed_block_header_t *new_block = (mixed_block_header_t *)((char *)curr + MIXED_HEADER_SIZE + aligned_size);
        new_block->size = curr->size - aligned_size - MIXED_HEADER_SIZE;
        new_block->is_free = true;
    ...
    // Update stats
    currently_allocated += curr->size + MIXED_HEADER_SIZE;

    // Return pointer
    return (void *)((char *)curr + MIXED_HEADER_SIZE);
}

void *mixed_best_fit_malloc(size_t size) {
    if (size <= 0) return NULL;
    ...
    mixed_block_header_t *curr = free_list_head;
    mixed_block_header_t *best_block = NULL;
...
void *mixed_worst_fit_malloc(size_t size) {
    if (size <= 0) return NULL;
    ...
    mixed_block_header_t *curr = free_list_head;
    mixed_block_header_t *worst_block = NULL;
...
void mixed_free(void *ptr) {
    ...
    mixed_block_header_t *header = (mixed_block_header_t *)((char *)ptr - MIXED_HEADER_SIZE);
    ...
    // Update stats
    currently_allocated -= (header->size + MIXED_HEADER_SIZE);
    ...
    if (curr && (mixed_block_header_t *)((char *)header + MIXED_HEADER_SIZE + header->size) == curr) {
        header->size += MIXED_HEADER_SIZE + curr->size;
        header->next_free = curr->next_free;
        if (curr->next_free) curr->next_free->prev_free = header;
    }

    // Coalesce with previous physical block
    if (prev && (mixed_block_header_t *)((char *)prev + MIXED_HEADER_SIZE + prev->size) == header) {
        prev->size += MIXED_HEADER_SIZE + header->size;
